...
static void Display(void)
{
    ...
    GLdouble p[16];
	GLdouble m[16];
	int count_d = 0; // cuenta cuantos hemos dibujado
	...
	for (i = 0; i < gObjectDataCount; i++) {
		...
		if ((gObjectData[i].visible != 0) && (gObjectData[i].vrml_id >= 0)) {
			...
			printf("id %d \n",gObjectData[i].id);
			id_reconocidos[count_d]=gObjectData[i].id;
			count_d++;
		}			
	}
	...
	if (count_d>=2){
		// Verificamos que el objeto reconocido corresponda con el objeto de la letra reconocida
		int idLetra = id_reconocidos[0];
		int idObjeto= id_reconocidos[1];
		int n_letras= gObjectDataCount/2;
		int id_correcto = gObjectData[idLetra+n_letras].id;

		if(id_correcto==idObjeto){
			printf("Felicitaciones \n");
			if (PlaySound(TEXT("C:\\Program Files\\ARToolKit\\examples\\simpleVRML\\wavesh.wav"), NULL,SND_ASYNC)){
...
int main(int argc, char** argv)
{
	...
	// Creamos arreglos de ID reconocidos
	if ((id_reconocidos = (int *)malloc(sizeof(int) * (4))) == NULL) exit (-1);
	if ((id_asociacion = (int *)malloc(sizeof(int) * (26))) == NULL) exit (-1);
